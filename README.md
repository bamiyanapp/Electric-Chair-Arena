...
| startMatch | `/start-match` | POST | 人間対AIの新規ゲームを開始する。 |
| playTurn | `/play-turn` | POST | プレイヤーの行動（選択/設置）を送信し、ゲームを1ターン進め、AIの行動結果および感電判定を返す。 |
| saveMatchAndTrain | `/save-match` | POST | 試合終了時にゲームログを保存し、その対戦ログを用いて対象AIモデルの学習処理（重み更新・レーティング変動）を実行する。 |
...
#### 1. players
AIモデルを格納するテーブル。学習パラメーターや戦績を保持する。

| 属性名 | 型 | キー | 説明 |
| :--- | :--- | :--- | :--- |
| playerId | String | Partition Key | プレイヤーの一意識別子 |
| name | String | - | AIモデル名 |
| type | String | - | プレイヤータイプ（random, rule_based, personality, llm, learning） |
| rating | Number | - | ELOレーティング（初期値1500） |
| winCount | Number | - | 人間に対する勝利数 |
| lossCount | Number | - | 人間に対する敗北数 |
| weights | Map | - | 行動最適化用の重みパラメーター（学習型等で利用） |
| updatedAt | String | - | 更新日時 (ISO8601) |

#### 2. matches
対戦結果と、各ターンの詳細ログを格納するテーブル。AIの継続的な強化学習・傾向分析に利用。

| 属性名 | 型 | キー | 説明 |
| :--- | :--- | :--- | :--- |
| matchId | String | Partition Key | 対戦の一意識別子 (UUID) |
| player1Id | String | - | プレイヤー1のID (人間の場合は `"human"`) |
| player2Id | String | - | プレイヤー2のID (AIモデルID) |
| scores | Map | - | 最終得点 (p1, p2) |
| shocks | Map | - | 最終感電回数 (p1, p2) |
| winnerId | String | - | 勝利したプレイヤーのID |
| ratingDiff | Number | - | この試合によるAIモデルのレーティング変動幅 |
...
## 行動パラメーターの最適化
対戦結果とターン履歴を基に、AIが「何点目の椅子に電流をセットされやすいか」「相手が何点目の椅子を好んで選ぶか」の確率テーブルを学習・更新。
...
本プロジェクトのフロントエンドは GitHub Actions を利用して自動的に GitHub Pages へデプロイされます。デプロイが 404 エラー（`Failed to create deployment`）で失敗する場合は、以下の設定が正しく行われているか確認してください。
...
## GitHub Pages へのデプロイ設定
本プロジェクトのフロントエンドは、`gh-pages` ブランチを介して GitHub Pages へデプロイされます。
GitHub Actions の CD ワークフロー（`cd.yml`）が実行されると、フロントエンドのビルド成果物（`out/`）が自動的に `gh-pages` ブランチにプッシュされます。
...
1. `backend/seed.js`（または所定のシードスクリプト）を実行し、データベースに既定のAIモデル（ランダム、慎重、アグレッシブ等）を投入します。
