...
function HomeContent() {
  ...
  const viewParam = searchParams.get('view')?.toUpperCase() as 'LOBBY' | 'SIMULATOR' | 'RESULT' | 'GAME' | 'LEADERBOARD' | undefined;
  const initialView = viewParam && ['LOBBY', 'SIMULATOR', 'RESULT', 'GAME', 'LEADERBOARD'].includes(viewParam) 
    ? viewParam 
    : 'LOBBY';

  const [currentView, setCurrentView] = useState<'LOBBY' | 'SIMULATOR' | 'RESULT' | 'GAME' | 'LEADERBOARD'>(initialView);
  ...
  useEffect(() => {
    if (viewParam && ['LOBBY', 'SIMULATOR', 'RESULT', 'GAME', 'LEADERBOARD'].includes(viewParam)) {
