 ...
 *   それ以外はスコア差ベースのスカラー値)
 */
// selfScore等が終局条件(勝利得点到達・感電上限到達・椅子切れ)に達しているかを
// 判定し、達していればself視点の終局値(勝利/敗北/引き分け相当)を返す。
// 未終了の場合はundefinedを返す。
function resolveEndgameTerminalValue(selfScore, opponentScore, selfShocks, opponentShocks, remainingChairsCount) {
  ...
  if (remainingChairsCount > GAME_RULES.MIN_CHAIRS_TO_END) return undefined;
...
function solveEndgameValue(remainingChairs, chooserScore, chooserShocks, setterScore, setterShocks, chooserIsSelf, memo) {
  ...
  const terminalValue = resolveEndgameTerminalValue(selfScore, opponentScore, selfShocks, opponentShocks, remainingChairs.length);
  if (terminalValue !== undefined) return terminalValue;
...
function resolveOpponentModel(role, opponentHistory, remainingChairs) {
  ...
  return { opponentPreference, observedOpponentCount, useOpponentModel };
}

// 終盤(先読み)か中盤以前(状態非依存の実効価値ヒューリスティック)かに応じて、
// fictitiousPlayへ渡すvalueFn/trapCostFnを決定する。
function resolveValueFunctions(role, remainingChairs, selfScore, opponentScore, selfShocks, opponentShocks) {
...
// 設置者(役割 'set')の手を決定する。
function decideSetterMove(
  remainingChairs, numToSet, valueFn, trapCostFn,
  opponentPreference, useOpponentModel, isOpponentOneShockFromLosing,
  usedEndgameLookahead, observedOpponentCount
) {
...
// 選択者(役割 'choose')の手を決定する。
function decideChooserMove(
  remainingChairs, numToSet, valueFn, trapCostFn,
  opponentPreference, useOpponentModel, usedEndgameLookahead, observedOpponentCount
) {
...
function getNashMove(playerId, role, remainingChairs, matchState = {}) {
  ...
  const { opponentPreference, observedOpponentCount, useOpponentModel } =
    resolveOpponentModel(role, opponentHistory, remainingChairs);

  const { valueFn, trapCostFn, usedEndgameLookahead } =
    resolveValueFunctions(role, remainingChairs, selfScore, opponentScore, selfShocks, opponentShocks);

  if (role === 'set') {
    return decideSetterMove(
      remainingChairs, numToSet, valueFn, trapCostFn,
      opponentPreference, useOpponentModel, isOpponentOneShockFromLosing,
      usedEndgameLookahead, observedOpponentCount
    );
  }
  return decideChooserMove(
    remainingChairs, numToSet, valueFn, trapCostFn,
    opponentPreference, useOpponentModel, usedEndgameLookahead, observedOpponentCount
  );
}
